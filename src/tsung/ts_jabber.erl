...
parse_bidi(Data, State) ->
    ...
    case re:run(RcvdXml,"<stream:error>\s*<see-other-host\s+xmlns=[\"\']urn:ietf:params:xml:ns:xmpp-streams[\"\']>([^<]+)</see-other-host>") of
        {match,[_, {Start, Length}|_]} ->
            <<_:Start/binary, Redirect:Length/binary, _/binary>> = Data,
            {Host, Port} = parse_host_port(Redirect, State),
            ?DebugF("REDIRECT host: ~p port: ~p~n",[Host, Port]),
            {nodata, State#state_rcv{host=Host, port=Port, count=State#state_rcv.maxcount, protocol=gen_tcp}};
        _Else ->
...
parse_host_port(HostPort, State) ->
    Regex = case HostPort of
        <<"[", _/binary>> -> "\\[([^\\]]+)\\](:([0-9]+))?"; % IPv6
        _ -> "([^:]+)(:([0-9]+))?"
    end,
    case re:run(HostPort, Regex) of
        {match,[_, {HostStart, HostLength}, _, {PortStart, PortLength}]} ->
            <<_:HostStart/binary, Host:HostLength/binary, _/binary>> = HostPort,
            <<_:PortStart/binary, Port:PortLength/binary, _/binary>> = HostPort,
            {binary_to_list(Host), list_to_integer(binary_to_list(Port))};
        {match,[_, {HostStart, HostLength}]} ->
            <<_:HostStart/binary, Host:HostLength/binary, _/binary>> = HostPort,
            {binary_to_list(Host), State#state_rcv.port};
        nomatch ->
            {State#state_rcv.host, State#state_rcv.port}
    end.
